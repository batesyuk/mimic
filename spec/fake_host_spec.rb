...
describe "Mimic::FakeHost" do
  ...
  it "returns a StubbedRequest" do
    expect(@host.get("/some/path")).to be_kind_of(Mimic::StubbedRequest)
  end

  describe "StubbedRequest" do
    it "has a unique hash based on it's parameters" do
      host = Mimic::StubbedRequest.new(stub, "GET", "/path")
      expect(host.to_hash).to eq(Digest::MD5.hexdigest("GET /path"))
    end

    it "has the same hash as an equivalent request" do
      host_one = Mimic::StubbedRequest.new(stub, "GET", "/path")
      host_two = Mimic::StubbedRequest.new(stub, "GET", "/path")
  ...
  RSpec::Matchers.define :match_rack_response do |code, headers, body|
    match do |actual|
      (expect(actual[0]).to eq(code)) &&
      (expect(actual[1]).to include(headers)) &&
      (expect(actual[2]).to include(body))
    end
  end
end
